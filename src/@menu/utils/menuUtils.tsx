...
export const confirmUrlInChildren = (children: ChildrenType['children'], url: string): boolean => {
  ...
  if (isValidElement(children)) {
    const { component, href, exactMatch, activeUrl, children: subChildren } = (children as any).props
...
const processMenuChildren = (children: ReactNode, mapFunction: (child: ReactNode) => ReactNode): ReactNode => {
  return Children.map(children, child => {
    ...
    return (child as any).props?.menuData ? (
      <GenerateVerticalMenu menuData={(child as any).props.menuData} />
    ) : (
      mapFunction(child)
    )
  })
...
export const mapHorizontalToVerticalMenu = (children: ReactNode): ReactNode => {
  return Children.map(children, child => {
    ...
    const { children: childChildren, verticalMenuProps, ...rest } = (child as any).props
