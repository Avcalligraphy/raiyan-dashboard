...
import type { TestimonialsType } from '@/types/apps/ecommerceTypes'
...
type TestimonialsWithActionsType = TestimonialsType & {
  actions?: string
}

const fuzzyFilter: FilterFn<TestimonialsWithActionsType> = (row, columnId, value, addMeta) => {
  // Rank the item
...
// Column Definitions
const columnHelper = createColumnHelper<TestimonialsWithActionsType>()

const TestimonialsTable = ({ reviewsData }: { reviewsData?: TestimonialsType[] }) => {
  // States
  const [status, setStatus] = useState<TestimonialsType['status']>('All')
  const [rowSelection, setRowSelection] = useState({})
  const [allData, setAllData] = useState<TestimonialsType[]>(reviewsData ?? [])
  const [data, setData] = useState<TestimonialsType[]>(reviewsData ?? [])
  ...
  const columns = useMemo(
          ...
          <OptionMenu
            ...
            options={[
              ...
              },
              {
                ...
                menuItemProps: {
                  onClick: () => setAllData(prev => prev?.filter(review => review.id !== row.original.id) ?? []),
    ...
    ] as ColumnDef<TestimonialsWithActionsType>[],
    []
  )
